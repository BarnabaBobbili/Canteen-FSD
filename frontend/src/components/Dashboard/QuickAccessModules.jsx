...
const QuickAccessModules = ({ accessibleModules }) => {
  ...
  // Map module colors to new theme colors
  const colorMap = {
    'from-blue-500 to-blue-600': { bg: '#E0F2FE', icon: '#1570EF' },
    ...
    'from-indigo-500 to-indigo-600': { bg: '#E0F2FE', icon: '#1570EF' },
    ...
    <div className="mb-6 md:mb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {accessibleModules.slice(1, 5).map((module, index) => {
          ...
          const colors = colorMap[module.color] || { bg: '#E0F2FE', icon: '#1570EF' };
